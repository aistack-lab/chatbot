...
def search_for_issues(jql_str: str) -> str:
    ...
    try:
        issues = jira_client.search_issues(jql_str=jql_str)
    ...
    formatted_issues = []
    for i in issues:
        issue = jira_client.issue(i.key)  # type: ignore
        formatted_issues.append(f"Issue: {issue.key}\n{format_issue(issue.fields)}")

    return "\n\n---\n\n".join(formatted_issues)


def create_issue(summary: str, description: str, issuetype: Literal["Bug"] = "Bug"):
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        agent = Agent(model="gpt-4o-mini", tools=[search_for_issues])
